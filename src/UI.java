...
public class UI implements ActionListener, MouseListener{
	...
	// zmienne występujące w całej klasie
	...
	/**
	 * Uruchomienie aplikacji, połączenie z bazą danych i ustawienie początkowych ustawień w panelu głównym
	 */
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				...
				} catch (Exception e) { // w przypadku błędu w uruchamianiu zamknięcie połączenia z bazą danych
					e.printStackTrace();
	...
	private void initialize() {
		...
		// tworzenie tabeli do wyświetlania danych pacjenta
		table = new JTable();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(z == btnDodajPacjenta) { // przycisk dodaj pacjenta powoduje odblokowanie możliwoœci dodawania pacjenta do bazy danych
		...
		if(z == btnZapisz) { // sprawdzenie poprawności danych zapisywanych, wpisanie pacjenta do bazy i uaktualnienie tabeli wyświetlającej pacjentów
			...
			if(sprawdzDane()) {
				...
				if(bazaPacjentow.checkPesel(pesel)) {
					...
					JOptionPane.showMessageDialog(frame, "Taki PESEL już istnieje!");
				}

			}
		}
		// zapisywanie badania do bazy danych, sprawdzanie jego poprawności, przypisywanie go użytkownikowi z wybranej listy (pesel)
		if(z == btnZapisz_1) {
		...
		//usunięcie wybranego pacjenta wraz z jego badaniami, jeśli nie ma żadnych pacjentów w tabeli uniemożliwienie usunięcia
		...
		// anulowanie wpisu pacjenta powoduje zablokowanie możliwości dodania wpisu, aby umożliwić dodanie wpisu należy użyć przycisku dodaj pacjenta
		if(z == btnAnuluj) {
			zezwoleniePacjent(false);
			textName.setText("");
			textSurname.setText("");
		...
		// obliczenie wartoœci wybranego parametru dla osoby wybranej z listy, wyświetlany jest wykres danego parametru
		if(z == btnOblicz) {
			...
			try {
				...
				if(wybrane.equals("Ciśnienie skurczowe")) {
					sr = obliczSrednia(pesel_pacjenta, wybrane);
					min = znajdzMinimum(pesel_pacjenta, wybrane);
					max = znajdzMaksimum(pesel_pacjenta, wybrane);
					textMinimum.setText(Integer.toString(min));
				...
				if(wybrane.equals("Ciśnienie rozkurczowe")) {
					sr = obliczSrednia(pesel_pacjenta, wybrane);
					min = znajdzMinimum(pesel_pacjenta, wybrane);
					max = znajdzMaksimum(pesel_pacjenta, wybrane);
					textMinimum.setText(Integer.toString(min));
				...
				zezwolenieBadanie(false);
			} catch(Exception es) { // w przypadku braku danych z bazy dla danego pacjenta, wartości ustawiane są na zero, wykres się nie rysuje
				JOptionPane.showMessageDialog(frame, "Nie można obliczyć parametrów");
		...
		// anulowanie wpisywania powoduje usunięcie wartości z pól i zablokowanie możliwości dodania wpisu
		if(z == btnAnuluj2) {
			zezwolenieOblicz(false);
			zezwolenieBadanie(false);
			txtTetno.setText("");
	...
	// sprawdzenie czy słowo zawiera same litery, jeśli zawiera coś poza literą zwraca true
	...
	// sprawdzenie czy słowo zawiera same cyfry, jeśli zawiera coś poza cyfrą zwraca true
	...
	// sprawdzanie poprawności wprowadzanych danych pacjenta
